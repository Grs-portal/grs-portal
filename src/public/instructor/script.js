...
buildCoursesSidebar()
  ...
  function toast(msg) {
    ...
    t.className =
      "fixed bottom-4 right-4 bg-black text-white px-4 py-2 rounded-xl z-50";
    t.textContent = msg;
  ...
  function showModal(html) {
    ...
    bg.innerHTML = `
      <div class="bg-white rounded-2xl p-6 w-[95%] max-w-lg">
        ${html}
      </div>
    `;
  ...
  const Courses = (() => {
      ...
      container.innerHTML = `
        <div class="flex gap-6 fade-in">
          ...
          <aside class="w-72 glass rounded-2xl p-4 space-y-3">
            ...
            <button id="createCourseBtn" class="btn-primary w-full">
              + New Course
            </button>
          ...
          <section id="coursesContent"
            class="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          </section>
    ...
    function renderGrid() {
      ...
      if (!list.length) {
        content.innerHTML = `
          <div class="col-span-full text-center glass rounded-2xl p-8">
            <p>No courses yet</p>
            <button class="btn-primary mt-3" id="emptyCreate">
              Create first course
            </button>
          </div>
        `;
      ...
      list.forEach((c) => {
        ...
        card.className =
          "glass rounded-2xl overflow-hidden cursor-pointer hover:scale-[1.02] transition";

        card.innerHTML = `
          <img src="${c.cover || "https://picsum.photos/400"}"
    ...
    function renderChapters() {
      ...
      c.chapters.forEach((ch, i) => {
        ...
        row.className =
          "glass rounded-xl p-3 flex justify-between text-sm";

        row.innerHTML = `
          <span>${i + 1}. ${esc(ch.title)}</span>
          <button>✕</button>
        `;
