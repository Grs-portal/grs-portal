document.addEventListener("DOMContentLoaded", () => {
  ...
  qs("#userName") && (qs("#userName").textContent = displayName);
  ...
  function showModal(innerHTML) {
    ...
    modalBg.id = "modalBg";
    modalBg.className = "fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50";
    modalBg.innerHTML = `
  ...
  function setupNotificationsUI() {
    ...
    qs("#notifReadAll")?.addEventListener("click", async () => {
      const role = localStorage.getItem("role") || "";
  ...
  async function loadDashboard() {
    ...
    qsa(".deleteCourseBtn").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        const r = await fetch(`${API}/courses/${btn.dataset.id}`, { method: "DELETE", headers: actorHeaders() });
  ...
  async function loadHomework() {
    ...
    list.innerHTML = hw.map(h => `
      <div class="bg-white border border-[#A5C8A1]/60 p-4 rounded-xl shadow-sm flex justify-between items-start">
        <div>
          ...
          <p class="text-xs opacity-70 mt-2">By: ${esc(h.submitted_by || "N/A")} · ${esc(h.course || "")}</p>
    ...
    qsa(".deleteHomeworkBtn").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        const r = await fetch(`${API}/homework/${btn.dataset.id}`, { method: "DELETE", headers: actorHeaders() });
        if (!r.ok) return toast("Delete failed", "#b91c1c");
    ...
    table.innerHTML = (data || []).map(s => `
      <tr>
        ...
        <td class="px-6 py-4 text-right">
          <button data-id="${s.enrollment_id}" data-grade="${s.grade ?? ""}" class="editBtn text-sm px-3 py-1.5 border rounded-lg hover:bg-black/5">✏️</button>
        </td>
      </tr>
    `).join("");
  ...
  function openEditGradeModal(id, grade) {
    ...
    qs("#submitModal")?.addEventListener("click", async () => {
      ...
      if (isNaN(newGrade) || newGrade < 1 || newGrade > 10) return toast("Grade must be 1–10", "#b91c1c");
  ...
  // ---- Navigation load triggers (based on your existing nav system) ----
